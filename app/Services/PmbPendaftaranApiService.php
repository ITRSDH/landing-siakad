...
class PmbPendaftaranApiService
{
    protected $apiUrl;
    protected $timeout;
    ...
    /**
     * Get Pmb Pendaftaran data from API
     * API dirancang untuk single data, jadi tidak perlu ID
     */
    public function getPmbPendaftaran($id = null)
    {
        try {
            ...
            if ($response->successful()) {
                ...
                if (isset($data['data'])) {
                    ...
                    return ensure_pmb_object($data['data']);
                } elseif (isset($data['success']) && $data['success'] && isset($data['data'])) {
                    Log::info('Using data from success.data key');
                    return ensure_pmb_object($data['data']);
                } else {
                    Log::info('Using raw data');
                    return ensure_pmb_object($data);
                }
            }

            Log::warning('API PMB Pendaftaran returned non-successful status: ' . $response->status());
            return null;

        } catch (Exception $e) {
            Log::error('API PMB Pendaftaran Error: ' . $e->getMessage());
            return null;
        }
    }
     ...
     * Karena API hanya mengembalikan satu data, ini sama dengan getPmbPendaftaran()
     */
    public function getActivePmbPendaftaran()
    {
        // Karena API sudah dirancang untuk single data, langsung ambil dari endpoint utama
        return $this->getPmbPendaftaran();
    }
    ...
    {
        try {
            $response = Http::timeout(10)->get($this->apiUrl . '/pmb-pendaftaran');
